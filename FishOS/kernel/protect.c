...
PRIVATE void init_descriptor(DESCRIPTOR	*p_desc, t_32 base, t_32 limit, t_16 attribute) {
	p_desc->limit_low	= limit & 0x0FFFF;			// limit of segment1(2 bytes)
	p_desc->base_low	= base	& 0x0FFFF;			// base of segment1(2 bytes)
	p_desc->base_mid	= (base >> 16) & 0x0FF;		// base of segment2(1 byte)
	p_desc->attr1		= attribute & 0xFF;			// attribute1
	p_desc->limit_high_attr2	= ((limit >> 16) & 0x0F) | (attribute >> 8) & 0xF0;	// limit2 + attribute2
	p_desc->base_high	= (base >> 24) & 0xFF;		// base of segment3(1 byte)
}
...
PUBLIC void exception_handler(int vec_no, int err_code, int eip, int cs, int eflags) {
	...
	// 通过打印空格的方式清空屏幕的前5行, 并把disp_pos清零
	disp_pos = 0;
	for (i = 0; i < 80 * 5; ++i) {
		disp_str(" ");
	}
...
// for the absolute address of segment
...
PUBLIC void init_prot() {
	init_8259A();
	// All are initialized into interrupt gate(no trap gate)
	...
	init_descriptor(&gdt[INDEX_LDT_FIRST], vir2phys(seg2phys(SELECTOR_KERNEL_DS), proc_table[0].ldts), LDT_SIZE * sizeof(DESCRIPTOR), DA_LDT);
